...
logging.basicConfig(format="%(asctime)s [%(levelname)s] %(funcName)s %(message)s", datefmt='%Y-%m-%d %H:%M:%S', filename=log_filename, level=logging.INFO)
...
class Chunk(Base):
    ...
    #controllers_chunk is a dictionary where the keys are the ids of the controller and the value is a list of the chunk in that controller 
    #in the form "{controllerid1:[chunkid,...],controllerid2:...}"
    controllers_chunk = Column(String)
    #controllers_phys is a dictionary where the keys are the ids of the controller and the value is a list of the chunk in that controller 
    #in the form "{controllerid1:[chunkid,...],controllerid2:...}"
...
class Box(Base):
    ...
    box_id_controller = Column(Integer)
    name = Column(String)
...
class Phy(Base):
    ...
    id = Column(Integer, primary_key=True)
    controller_id = Column(Integer)
    phy_id_controller = Column(Integer)
    type = Column (String)
    name = Column(String)
...
class Service(Base):
    ...
    id = Column(Integer, primary_key=True)
    #controllers_services is a dictionary where the keys are the ids of the controller and the value is a list of the chunk in that controller 
...
def _dictChunk(chunk):
    services = session.query(Service).filter(Service.id.in_(eval(chunk.serviceList))).all()  
...
def _dictPhy(phy):
    vifs = session.query(Vif).filter(Vif.id.in_(eval(phy.virtualInterfaceList))).all()
...
# -I2CAT type controller initial topology recovered from live SOUTHBOUND REQUEST to controller
# 
# CURRENT STATE (Chunks, Services, VirtualInterfaces):
# ==============
# -RUCKUS type controller current state recovered from database and controllers runtime status
        ...
        for db_chunk in db_chunks:
            if eval(db_chunk.controllers_chunk)[len(controllers)]:
                ...
                for service in chunk["serviceList"]:
                    db_service = session.query(Service).filter(Service.id == service["id"]).one()
                    if len(controllers) in eval(db_service.controllers_services).keys():
                        service["id"] = eval(db_service.controllers_services)[len(controllers)]
                    else:
                        services_to_pop.append(service)
                [chunk["serviceList"].remove(service) for service in services_to_pop]
                
                for phy in chunk["physicalInterfaceList"]:
                    try:
                        db_phy = session.query(Phy).filter(Phy.id == phy["id"], Phy.controller_id == len(controllers)).one()
                        phy = db_phy.phy_id_controller
                    except NoResultFound:
                        phys_to_pop.append(phy)
                [chunk["physicalInterfaceList"].remove(phy) for phy in phys_to_pop]

                chunk["id"] = eval(db_chunk.controllers_chunk)[len(controllers)]
                chunks.append(chunk)
    ...
    for box in controller.getChunketeTopology()[0]["boxes"]:
        if box['id'] not in [r for (a,r) in db_id_box_id_list]:
            try:
                ...
                new_box = Box(id=new_box_id, name=box["name"], location=json.dumps(box["location"]), controller_id=item['id'], box_id_controller=box['id'], phys=json.dumps(box["phys"]), box_json = json.dumps(box))
                session.add(new_box)
                count_phys= 0
                for phy in box["phys"]:
                    if phy['id'] not in [r for (a,r) in db_id_phy_id_list]:
                        new_phy = Phy(id=new_phy_id+count_phys, name=phy["name"], type=phy["type"], controller_id=item['id'], phy_id_controller=phy['id'], config= str(phy["config"])
                        , virtualInterfaceList=json.dumps([]), phy_json = json.dumps(phy))
                        session.add(new_phy)
...
def flaskResponse(response):
    ...
    log_content = " '{}' {} :code:{}:body:{}".format(request.method, request.path, response.status_code,body)    
...
def getChunketeTopology():
    ...
    for index_controller in range(len(controllers)):      
        try:
            boxes = session.query(Box).filter(Box.controller_id == index_controller).all()
            (controller_resp, code) = controllers[index_controller].getChunketeTopology()
            log_content = "controller:{}:response:{}/{}".format(index_controller,code,controller_resp)     
            ...
            if code == API_RESPONSE["OK"]["code"]:
                for box in controller_resp["boxes"]:
                    for index_phy in range(len(box["phys"])):
                            phy = session.query(Phy).filter(Phy.controller_id == index_controller).filter(Phy.phy_id_controller == box["phys"][index_phy]["id"]).one()
...
@app.route('/chunkete/topology/physicalInterface/<phy_id>/LTEConfig', methods=['PUT'])
def putInterfaceLTEConfig(phy_id):
    ...
    try:
        ...
        if 0 > content_dict["cellIdentity"] > 256:
            return errorResponder("VERIFICATION_ERROR", "Malformed request")
        if content_dict["earfcndl"] not in [i for j in (range(2750,3449), range(41690,43489), range(37750,38249)) for i in j]:
            return errorResponder("VERIFICATION_ERROR", "Malformed request")
        ...
        if content:
            ...
            response,code = controllers[phy.controller_id].putInterfaceLTEConfig(phy.phy_id_controller,content)
            log_content = "controller:{}:phy_id_controller:{}:phy_id:{}:content:{}:response:{}/{}".format(phy.controller_id,phy.phy_id_controller,phy_id,content,code,response)     
            logger.info(log_base.format(SOUTHBOUND,REQRESP,log_content))
...
@app.route('/chunkete/topology/physicalInterface/<phy_id>/type/<phy_type>', methods=['PUT'])
def putInterfaceType(phy_id,phy_type):
    try:
        log_content = "phy_id:{}:phy_type:{}".format(phy_id,phy_type)    
        logger.info(log_base.format(NORTHBOUND,REQUEST,log_content))
        phy = session.query(Phy).filter(Phy.id == phy_id).one()
        response, code = controllers[phy.controller_id].putInterfaceType(phy.phy_id_controller, phy_type)
        log_content = "controller:{}:phy_id_controller:{}:phy_id:{}:phy_type:{}:response:{}/{}".format(phy.controller_id,phy.phyid_controller, phy_id, phy_type,code,response)     
        logger.info(log_base.format(SOUTHBOUND,REQRESP,log_content))
...
@app.route('/chunkete/topology/physicalInterface/<phy_id>/wiredConfig', methods=['PUT'])
def putInterfaceWiredConfig(phy_id):
    try:
        content = request.data
        log_content = "phy_id:{}:content:{}".format(phy_id,json.loads(content))    
        logger.info(log_base.format(NORTHBOUND,REQUEST,log_content))
        if content:

                phy = session.query(Phy).filter(Phy.id == phy_id).one()
                response,code = controllers[phy.controller_id].putInterfaceWiredConfig(phy.phy_id_controller, content)
                log_content = "controller:{}:phy_id_controller:{}:phy_id:{}:response:{}/{}".format(phy.controller_id,phy.phy_id_controller, phy_id,code,response)     
                logger.info(log_base.format(SOUTHBOUND,REQRESP,log_content))
                return response, code
    except KeyError:
        return errorResponder("VERIFICATION_ERROR", "Malformed request")
...
@app.route('/chunkete/topology/physicalInterface/<phy_id>/wirelessConfig', methods=['PUT'])
def putInterfaceWirelessConfig(phy_id):
    ...
    try:
        ...
        if content_dict["channelBandwidth"] not in [20,40,80]:
            return errorResponder("VERIFICATION_ERROR", "Malformed request")
        if content_dict["channelNumber"] not in [i for j in (range(1,11),range(36,68,4), range(100,140,4)) for i in j]:
            return errorResponder("VERIFICATION_ERROR", "Malformed request")
        ...
        if content:
            phy = session.query(Phy).filter(Phy.id == phy_id).one()
            response,code = controllers[phy.controller_id].putInterfaceWirelessConfig(phy.phy_id_controller,content)
            log_content = "controller:{}:phy_id_controller:{}:phy_id:{}:content:{}:response:{}/{}".format(phy.controller_id,phy.phy_id_controller,phy_id,content,code,response)     
            logger.info(log_base.format(SOUTHBOUND,REQRESP,log_content))
            return jsonify(response),code
        else:
    ...
    try:
        ...
        for phy in chunk_dict["physicalInterfaceList"]:
            ...
            if phy.controller_id in controllers_phys.keys():
                controllers_phys[phy.controller_id].append(phy.phy_id_controller)
                controllers_content[phy.controller_id]["physicalInterfaceList"].append(phy_id_dict)
            else:
                ...
                if "assignedQuota" in chunk_dict.keys():
                    controllers_content[phy.controller_id]["assignedQuota"] = chunk_dict["assignedQuota"]
                else:
                    chunk_dict["assignedQuota"] = 0
                    controllers_content[phy.controller_id]["assignedQuota"] = 0
                if "linkList" in chunk_dict.keys():
                    controllers_content[phy.controller_id]["linkList"] = chunk_dict["linkList"]
                else:
                    chunk_dict["linkList"] = []
                    controllers_content[phy.controller_id]["linkList"] = []
                if "serviceList" in chunk_dict.keys():
                    controllers_content[phy.controller_id]["serviceList"] = chunk_dict["serviceList"]
                else:
        ...
        chunk = Chunk(id=new_chunk_id, name=chunk_dict["name"], serviceList=json.dumps([]), assignedQuota=chunk_dict["assignedQuota"], controllers_phys=str(controllers_phys), phyList=str([phy["id"] for phy in chunk_dict["physicalInterfaceList"]]), linkList=json.dumps([]), chunk_json=json.dumps(chunk_dict))
        session.add(chunk)
        
        #Register the chunk on each of the controllers        
        controllers_chunk_dict = {}
        for controller_id in controllers_content.keys():
            response, code = controllers[controller_id].registerNewChunk(json.dumps(controllers_content[controller_id]))
            log_content = "controller:{}:content:{}:response:{}/{}".format(controller_id,json.dumps(controllers_content[controller_id]),code,response)     
            logger.info(log_base.format(SOUTHBOUND,REQRESP,log_content))
            if code == API_RESPONSE["CREATED"]["code"]:
                controllers_chunk_dict[controller_id] = int(response["id"])
            else:
                return errorResponder("CONTROLLER", "Managed Controller returned an error")
            
        #Update Service in Database
        chunk_dict["id"] = new_chunk_id
        chunk.chunk_json = json.dumps(chunk_dict)
        chunk.controllers_chunk = str(controllers_chunk_dict)
        session.commit()            
        return json.dumps({'id':new_chunk_id}),API_RESPONSE["CREATED"]["code"] 
    except KeyError:
        return errorResponder("VERIFICATION_ERROR", "Malformed request")
    except NoResultFound:
        return errorResponder("NOTFOUND", "Item not found")
...
def getChunkById(chunk_id):
    ...
    try:
        ...
        return errorResponder("NOTFOUND", "Object not found")
    except IntegrityError:
        return errorResponder("DB_INTEGRITY", "Database integrity error")
    finally:
        session.close()
...
def removeExistingChunk(chunk_id):
    ...
    try:
        ...
        while serviceList:
            removeExistingSWAMService(chunk_id, serviceList[0], interface=INTERNAL)
            serviceList.pop(0)

        for controller_id in controllers_phys.keys():             
            response, code = controllers[controller_id].removeExistingChunk(eval(chunk.controllers_chunk)[controller_id])
            log_content = "controller:{}:chunk_id:{}:response:{}/{}".format(controller_id,chunk_id,code,response)     
            logger.info(log_base.format(SOUTHBOUND,REQRESP,log_content))
...
def getAllSWAMServices(chunk_id):
    ...
    try:
        ...
        for service_id in eval(db_chunk.serviceList):
            db_service = session.query(Service).filter(Service.id == service_id).one()
...
def registerNewSWAMService(chunk_id):
    ...
    #     "plmnId": "00101"
    # },
    # "selectedPhys": [ (Sólo se aceptan interfaces de tipo SUB6_ACCESS, LTE_PRIMARY_PLMN y WIRED_TUNNEL)
    #     14, 23
    # ],
    # "vlanId": 201, (1-4095)
    # "wirelessConfig": {
    #     "encryption": "WPA", (NONE, WPA, WPA2, WEP aceptados)
    #     "password": "secret", (No se aceptan espacios. Debe contener un mínimo de 8 caracteres o estar vacia en caso de encryption == "NONE")
    ...
    try:
        content = request.data
        log_content = "chunk_id:{}:content:{}".format(chunk_id,json.loads(content))    
        logger.info(log_base.format(NORTHBOUND,REQUEST,log_content))
        if content:
            service_dict = json.loads(content)

            if "lteConfig" not in service_dict.keys():
            ...
            if "wirelessConfig" not in service_dict.keys():
                service_dict["wirelessConfig"]= {
                    "encryption": None,
                    "password": None,
                    "ssid": None
                }
            elif service_dict["wirelessConfig"]["encryption"] not in ENCRYPTION_TYPES:
                return errorResponder("VERIFICATION_ERROR", "Malformed request")
            ...
            for phy_id in service_dict["selectedPhys"]:               
                ...
                #TODO:Name the new vif. At the moment, it just takes the phy name followed by the new_vif_id
                new_vif_name = "{}_{}".format(phy.name,new_vif_id)
                ...
                vif = Vif(id=new_vif_id, service_id=new_service_id, phy_id=phy.id, controller_id=phy.controller_id, vif_json = json.dumps(new_vif_dict))
                session.add(vif)
                selected_vifs.append(new_vif_id)
                phy = session.query(Phy).filter(Phy.id == phy.id).one()
            ...
            serviceList.append(new_service_id)
            chunk.serviceList = json.dumps(serviceList)
                #update chunk json
            service_dict["id"] = new_service_id
            ...
            vlan = Vlan(id=service_dict["vlanId"], service_id=new_service_id, controllers_vlans_id= "")
            ...
            for controller_id in controllers_phys.keys():
                data,code = controllers[controller_id].registerNewSWAMService(eval(chunk.controllers_chunk)[controller_id],json.dumps(controllers_content[controller_id]))
                log_content = "controller:{}:chunk_id:{}:content:{}:response:{}/{}".format(controller_id,chunk_id,json.dumps(controllers_content[controller_id]),code,data)     
                logger.info(log_base.format(SOUTHBOUND,REQRESP,log_content))
                if code == API_RESPONSE["CREATED"]["code"]:
                    rollback[controller_id] = {
                        'chunk_id': eval(chunk.controllers_chunk)[controller_id],
                        'service_id': int(data["id"])
                    }
                    controllers_services_dict[controller_id] = int(data["id"])
                else:
                    return errorResponder("CONTROLLER", "Managed Controller returned an error")
            
            #Update and add vlan object
            vlan.service_id = new_service_id
            
            #Update Service in Database
            service.controllers_services = str(controllers_services_dict)
            session.commit()            
            rollback_flag = False
            return json.dumps({'id': new_service_id}),API_RESPONSE["CREATED"]["code"] 
    except NoResultFound:
        return errorResponder("NOTFOUND", "Item not found")
    except IntegrityError:
        return errorResponder("DB_INTEGRITY", "Database integrity error")
        ...
        if rollback_flag == True:
            if rollback:
                for controller_id in rollback.keys():
                    data, code = controllers[controller_id].removeExistingSWAMService(rollback[controller_id]["chunk_id"],rollback[controller_id]["service_id"])
                    log_content = "controller:{}:chunk_id:{}:service_id:{}:response:{}/{}".format(controller_id,rollback[controller_id]["chunk_id"],rollback[controller_id]["service_id"],code,data)     
                    logger.info(log_base.format(SOUTHBOUND,ROLLBACK,log_content))
        session.close() 

@app.route('/chunkete/chunk/<chunk_id>/service/SWAM/<service_id>', methods=['GET'])
...
@app.route('/chunkete/chunk/<chunk_id>/service/SWAM/<service_id>', methods=['DELETE'])
def removeExistingSWAMService(chunk_id, service_id, interface=NORTHBOUND):
    ...
    try: 
        #Update Chunk in database
        chunk = session.query(Chunk).filter(Chunk.id == int(chunk_id)).one()
        vifs = session.query(Vif).filter(Vif.service_id == int(service_id)).all()
        ...
        for index in range(len(serviceList)):
            if serviceList[index] == int(service_id):
                service = session.query(Service).filter(Service.id == int(service_id)).one()
                controllers_services_dict = eval(service.controllers_services)
                for controller_id in controllers_phys.keys():             
                    response, code = controllers[controller_id].removeExistingSWAMService(eval(chunk.controllers_chunk)[controller_id],controllers_services_dict[controller_id])
                    log_content = "controller:{}:chunk_id:{}:service_id:{}:service_id_controller:{}:response:{}/{}".format(controller_id,chunk_id,service_id,controllers_services_dict[controller_id],code,response)     
                    logger.info(log_base.format(SOUTHBOUND,REQRESP,log_content))

                chunk_dict["serviceList"].pop(index)
                serviceList.pop(serviceList.index(int(service_id)))
                chunk.serviceList = json.dumps(serviceList)
                chunk.chunk_json = json.dumps(chunk_dict)
                
                vlan = session.query(Vlan).filter(Vlan.service_id == int(service_id)).one()
                session.delete(vlan)
